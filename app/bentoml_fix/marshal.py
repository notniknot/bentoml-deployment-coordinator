...
def metrics_patch(cls):
    class _MarshalService(cls):
        ...
        def __init__(
            self,
            *args,
            namespace: str = Provide[BentoMLContainer.config.bento_server.metrics.namespace],
            **kwargs,
        ):
        ...
        async def _batch_handler_template(self, requests, api_route, max_latency):
            ...
            self.metrics_request_batch_size.labels(endpoint=api_route).observe(len(requests))
...
class MarshalService:
    ...
    def __init__(
        ...
        mb_max_latency: int = Provide[BentoMLContainer.config.bento_server.microbatch.max_latency],
        max_request_size: int = Provide[BentoMLContainer.config.bento_server.max_request_size],
        outbound_unix_socket: str = None,
        enable_access_control: bool = Provide[BentoMLContainer.config.bento_server.cors.enabled],
        ...
        if self._client is None or self._client.closed:
            ...
            if self.timeout:
                timeout = aiohttp.ClientTimeout(
                    total=None, sock_read=self.timeout, sock_connect=self.timeout
                )
            else:
                timeout = None
    ...
    def setup_routes_from_pb(self, bento_service_metadata_pb):
        for api_pb in bento_service_metadata_pb.apis:
            if api_pb.batch:
                max_latency = self.mb_max_latency or api_pb.mb_max_latency or DEFAULT_MAX_LATENCY
                max_batch_size = (
                    self.mb_max_batch_size or api_pb.mb_max_batch_size or DEFAULT_MAX_BATCH_SIZE
                )
                self.add_batch_handler(api_pb.route, max_latency, max_batch_size)
                logger.info(
                    "Micro batch enabled for API `%s` max-latency: %s" " max-batch-size %s",
                    api_pb.route,
                    max_latency,
                    max_batch_size,
                )
    ...
    async def _batch_handler_template(self, requests, api_route, max_latency):
        ...
        with get_tracer().async_span(
            ...
            try:
                ...
                raise RemoteException(
                    e,
                    payload=HTTPResponse(status=500, body=b"Cancelled before upstream responses"),
                )
            ...
            return tuple(
                Response(body=i.body, headers=i.headers, status=i.status or 500) for i in merged
            )
    ...
    def make_app(self) -> "Application":
        ...
        if self.enable_access_control:
            ...
            cors = aiohttp_cors.setup(
                app,
                defaults={self.access_control_allow_origin: self.access_control_options},
            )
